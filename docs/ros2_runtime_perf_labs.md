...
所以**默认选 Pi 4（8 GB）**：它是候选里唯一带硬件编码器的，后期加摄像头时不用二次换板；内存买大的，需要压力时再用 cgroup 掐。它的性能弱不是问题——x86 主机已经充当"强机对照组"，Pi 的职责本来就是当受限目标。
...
### C4 V4L2 M2M 编码器与 DMA-BUF 零拷贝（需要 Pi 4，见 §1.1）

串口驱动只能练到"读字节"这一层，这个实验才练得到现代 Linux 媒体子系统的核心：**缓冲区所有权与跨设备零拷贝**。

- **对象**：CSI 摄像头 → ISP → 硬件 H.264 编码器（Pi 4 上是 `bcm2835-codec`，走标准 V4L2 M2M 接口）。
