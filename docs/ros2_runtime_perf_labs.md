...
| --- | --- | --- | --- |
| 1 | **§2.2 三设备时间同步** | 时间轴不可信，后面所有实验的数字都不可信（**但这条只对 SLAM 与跨设备端到端延迟成立，对 runtime 实验不成立——见 §9.3**） | 数据同步、驱动、硬件时间戳 |
...
### 9.3 只有 PC + 两块 ESP32 时的执行顺序

设备：x86 Ubuntu 主机（顶替 Pi 当上位机）、ESP32-A（雷达采集发布）、ESP32-B（电机 + 200 Hz 控制环）、两者之间一根 GPIO 同步线。

**先纠正 §9 开头的一条排序依据。** 那里把时间同步排第一，理由是"时间轴不可信则后面数字不可信"——这话对 **SLAM 和跨设备端到端延迟**成立，对 **runtime 架构实验不成立**。executor 抖动、回调时长、进程划分收益，全部在主机的单一时钟上测量，不需要任何跨设备同步。**所以把 runtime 提前是可以的。**

链路实验（A2/A5）确实想要单向延迟，但可以用**主机单钟的往返测量**替代：主机发、ESP32 回显、主机记 RTT，同样不需要同步。

#### 四个阶段

| 阶段 | 做什么 | 设备要求 |
| --- | --- | --- |
| **0 标尺** | §1.5 锁频基线 + §1.3 打点的主机段 | 只要 PC。**跳过这步后面全白做** |
| **1 runtime 架构** | B1 → B2 → §2.3 → A3（机制部分） | 只要 PC。负载先用合成（§9.2），ESP32-A 一接上就换真数据 |
| **2 低延迟链路** | §2.5 的 10 ms `vTaskDelay` → A2 分片 → A5 尾延迟 → C1 串口 | PC + 两块 ESP32 |
| **3 加分项与其余** | B5 → B3 → E1 → §2.2 时间同步 | 同上 |

阶段 2 的第一项之所以排在最前，是因为它是**纯收益**：删掉一个无条件延迟，`cmd_vel` 时延直接下降，不需要任何权衡讨论，适合用来跑通整套测量流程。

时间同步排到最后不是因为不重要，而是**它服务的是 SLAM，而 SLAM 现在不是重点**。它只要两块 ESP32 加一根线，随时可以插进来，从头到尾不依赖 Pi。

#### x86 上要注意的两件事

**一、限制核数是合理的，注入负载不是。** 用 `taskset -c 0,1` 或 cgroup 把整套 ROS 进程压到 2 个核——这改的是**结构参数**（可用并行度），而 executor 的行为本来就由它决定，不算伪造。反过来用 `stress-ng` 占 CPU 来"模拟弱板"就是伪造，别做（§1.1）。这条界线值得记住：**改结构参数可以，造假负载不行。**

**二、别动 PC 的系统。** 两个加分项在 x86 上都没有版本冲突：`linux-generic` 默认带 BTF（BTF 缺失是 `linux-raspi` 独有的坑），RT 内核在 22.04 和 24.04 上都能 `sudo pro enable realtime-kernel` 装 generic 变体。先各验一句：

```bash
ls /sys/kernel/btf/vmlinux && echo "BTF OK"        # eBPF 能不能用
uname -v | grep -q PREEMPT_RT && echo "RT 已启用"
```

§1.1 那套 24.04 + Jazzy 的结论**只针对树莓派**。PC 上现成的 Humble 继续用，不要为了统一版本去折腾主力机。

#### 等 Pi 到位后需要补什么

只有三类：B4 缺页与 D2 IO 抖动（资源性，x86 上做不真——不过 D2 可以先用一个廉价 USB 2.0 U 盘当慢盘，随机写够慢，能提前跑通）、ARM 弱内存序复验、以及把阶段 1~2 的脚本原样重跑拿 ARM 数字。**前面的结论到时候只需要换一组数，不需要重做。**
