...
> **实车从哪读：** 下文公式与缺口 1–6 来自仿真（\(M=0.756\)、\(r=0.0325\)、`lqr_backend.py`）。固件是 `pio run -e stage5`，植物理以 `include/config.h` 为准。**开展流程见 [`stage6.md`](stage6.md)。** 不要把仿真 `c=3.916` 写进 ESP32。
...
补偿这个扰动，恰好需要两个动作，对应坡上平衡的两个特征（推导见 [§定点 $\tau_{\mathrm{eq}}$](#tau-eq-derive)、[§$\theta_{\mathrm{eq}}$](#theta-eq-derive)）：
...
控制里用估计角 $\hat{\alpha}$ 代替真值 $\alpha$，把下面推出的定点保持力矩 $\tau_{\mathrm{eq}}$ **开环、即时**送出去，不等 LQR 走「发现下滑 → 误差 → 反馈纠正」那条慢链路。

### 定点推导：为何 $\tau_{\mathrm{eq}}=Mgr\sin\alpha$ {#tau-eq-derive}

考虑车在倾角为 $\alpha$ 的斜坡上**定点悬停**：沿坡速度 $v=0$、加速度 $\dot v=0$，姿态也已停在平衡俯仰（下一节的 $\theta_{\mathrm{eq}}$）。摩擦项 $c v$ 此时为 0。把整车看成沿坡方向的一维质点（轮缘提供驱动力），做准静态力平衡。

**步骤 1 · 重力沿坡分量**

取沿坡向上为正（与「上坡方向」一致；符号约定与实现一致即可，这里取「抵消下滑」所需的向上力）。重力 $Mg$ 竖直向下，沿坡方向的分量为：

$$
F_{g,\parallel} = Mg\sin\alpha
$$

方向沿坡**向下**（要滑下去）。无驱动力时，车会沿坡加速下滑。

**步骤 2 · 轮缘力与轮轴力矩**

电机在轮轴上施加总力矩 $\tau$（左右轮之和）。理想滚动、无打滑时，地面给轮缘的切向力为：

$$
F_{\mathrm{drive}} = \frac{\tau}{r}
$$

方向沿坡（由 $\tau$ 的符号决定）。要顶住下滑，需要沿坡**向上**的力，大小至少等于 $Mg\sin\alpha$。
...
$$
\frac{\tau_{\mathrm{eq}}}{r} = Mg\sin\alpha
\quad\Longrightarrow\quad
\boxed{\tau_{\mathrm{eq}} = M g r \sin\alpha}
$$

这就是坡上悬停所需的**持续总轮力矩**。前馈令 $\tau_{\mathrm{ff}}=\tau_{\mathrm{eq}}(\hat{\alpha})$，用估计角代入同一公式。

**步骤 4 · 与动态通道的关系（对照用）**

有运动时，同一条沿坡牛顿式还要加上惯性与阻尼：

$$
\frac{\tau}{r} \approx \tilde m\,\dot v + c\,v + Mg\sin\alpha
$$

定点令 $\dot v=0,\,v=0$，立刻退回 $\tau_{\mathrm{eq}}=Mgr\sin\alpha$。估计器里的 $\sin_{\mathrm{dyn}}$ 是把该式对 $\sin\alpha$ 求解；前馈是把定点特解直接当已知扰动补偿——**同一物理、两个用法**。

**步骤 5 · 符号与单位（本车）**

| 量 | 含义 | 本车典型值 |
| --- | --- | --- |
| $M$ | 整车质量（含轮） | $0.756\,\mathrm{kg}$ |
| $r$ | 轮半径 | $0.0325\,\mathrm{m}$ |
| $g$ | 重力加速度 | $9.8\,\mathrm{m/s^2}$ |
| $Mgr$ | 合成系数 | $0.2408\,\mathrm{N\cdot m}$ |
| $\alpha=10^\circ$ | $\sin\alpha\approx0.174$ | $\tau_{\mathrm{eq}}\approx0.0418\,\mathrm{N\cdot m}$ |

> **约定：** 上文 $\tau_{\mathrm{eq}}$ 是**整车总轮力矩**。实现里左右平分时，单轮前馈为 $0.5\,Mgr\sin\hat\alpha$（见 `lqr_backend` 的 `tau_ff_wheel`）。
...
$M=0.756$、$r=0.0325$、$g=9.8$：$MGR=M\cdot g\cdot r=\mathbf{0.2408}$，故 $\tau_{\mathrm{ff}}=0.2408\cdot\sin\hat{\alpha}$。10° 坡：$\tau_{\mathrm{ff}}\approx\mathbf{0.0418\mathrm{N\cdot m}}$——与上一节算出的 $\tau_{\mathrm{eq}}$ 一致。

> **只做到这一步会怎样？** 车能获得 holding 力矩、不至于一上坡就下滑；但由于 LQR 参考仍是 $\theta=0$，它会把坡上自然的前倾 $\theta_{\mathrm{eq}}$ 当成误差去纠正——导致**稳态位置漂移或持续的姿态-位置拉锯**。所以平衡点搬移不是可选项（推导见下一节）。
...
### 为什么坡上平衡姿态不是竖直的 {#theta-eq-derive}

上一节只保证了**沿坡平动**不滑；车体还是倒立摆，电机力矩有反作用。

**步骤 1 · 反作用力矩**

轮轴上施加 $\tau$（对轮「正转顶坡」），由牛顿第三定律，车体受到绕轮轴的反作用：

$$
Q_\theta = -\tau
$$

定点时 $\tau=\tau_{\mathrm{eq}}=Mgr\sin\alpha$，故车体持续受到大小为 $Mgr\sin\alpha$、方向要把车体**往后掀**的力矩。

**步骤 2 · 重力提供的回复力矩**

车体相对竖直（或相对坡法向，小角度下与实现约定一致）前倾 $\theta$，质心在轮轴上方距离 $L$，车体质量 $m_b$。重力对轮轴的力矩为：

$$
\tau_{\mathrm{grav}} = m_b g L \sin\theta
$$

方向：前倾时产生把车体「往前压」的力矩，可与反作用 $Q_\theta$ 平衡。
...
\quad\Longrightarrow\quad
\boxed{\sin\theta_{\mathrm{eq}} = \frac{M r}{m_b L}\sin\alpha}
$$

定义 $K_{\mathrm{EQ}}=Mr/(m_b L)$。你的车 $r\approx L$ 且质量分配使 $K_{\mathrm{EQ}}\approx 1$，故 **$\theta_{\mathrm{eq}}\approx\alpha$**：坡多陡，车体就要前倾多少。这不是控制误差，是坡上唯一的真实平衡姿态。

**步骤 4 · 与前馈的配对**

同一份 $\tau_{\mathrm{eq}}$ 出现两次：沿坡力平衡要它（$\tau_{\mathrm{ff}}$），俯仰力矩平衡也要它（决定 $\theta_{\mathrm{eq}}$）。所以实现里必须用同一个 $\sin_{\mathrm{eff}}$ 生成两半——拆开就会「力与参考不认账」（结论 4.1）。
...
### 频带分离：斜坡补偿走低频，平衡走高频 {#freq-separation}

这是本卷最重要的**系统级**知识点之一：不是再调一个增益，而是规定「谁管快、谁管慢」，两套动作共用一个力矩口，却不抢带宽。

#### 知识点属于哪里

| 说法 | 通常归属 |
| --- | --- |
| 快环 / 慢环、时间尺度分离 | 经典控制、级联控制、奇异摄动（two-time-scale） |
| 扰动慢 → 用慢估计 + 前馈补偿 | 扰动观测 / 前馈控制 |
| 入口低通、死区压回路增益 | 信号与系统、抗自激工程手段 |

LQR 决定**快环**的极点；斜坡链只允许走**慢通道**。死区与一阶低通的公式细节见 [α̂ 入环前的死区与一阶低通](./alpha_lpf_deadband.md)。

#### 一句话定义

**高频管平衡，低频管坡度。**  
同一拍输出仍是 \(\tau=\tau_{\mathrm{fb}}+\tau_{\mathrm{ff}}\)，但：

- \(\tau_{\mathrm{fb}}=-K e\)：闭环、快，专门稳住 \(\theta,\dot\theta\)（以及位置/速度）
- \(\tau_{\mathrm{ff}}\) 与 \(\theta_{\mathrm{eq}}(\hat\alpha)\)：开环性质、慢，只跟着环境坡度慢慢变

\(\hat\alpha\) **不穿过** \(K\)，所以不改平衡特征方程 \(A-BK\)（见上文「增益 K 一个字没动」）。频带分离保证的是：慢链上的噪声和滞后，搅不进快环的稳定裕度。
...
| 回路 | 带宽 / 时间常数 | 由谁驱动 |
| --- | --- | --- |
| 平衡（\(K\) 反馈 \(\theta,\dot\theta,\ldots\)） | 快，约几十 rad/s | 状态误差 |
| 斜坡估计（KF）+ 入环低通 | 慢；入环 \(\tau\approx 0.5\,\mathrm{s}\) | \(\hat\alpha\) |
| 坡本身变化 | 更慢（车速 × 地形） | 环境 |


\(\hat\alpha\) 只从 \(\tau_{\mathrm{ff}}\) 和 \(x_{\mathrm{ref}}\)（经 \(\theta_{\mathrm{eq}}\)）两个**低频入口**进入。即便 \(\hat\alpha\) 滞后零点几秒，最坏是上坡头前馈欠补，由快速 LQR 临时兜底——不会动摇平衡稳定性。

#### 落地靠什么（不只靠「KF 调慢」）

1. **估计器宁慢勿快**（KF 过程噪声 \(q\)、观测噪声 \(R\)）  
2. **控制器入口再低频化**：死区 + `slope_alpha_lpf_sec`（缺口 1）——估计慢是必要条件，**入环再滤才是充分条件**  
3. **禁止**把 \(\hat\alpha\) 塞进状态反馈当第五个增益去「压坡」
...
$$
\text{车动}\to\tau,\dot v\text{ 波动}\to z_{\mathrm{dyn}}\to\hat\alpha\to\theta_{\mathrm{eq}},\tau_{\mathrm{ff}}\to\text{车又动}
$$

这条回路在重叠频段上增益可能 \(>1\)，表现为平地来回蹭或坡上慢自激（缺口 1/5）。低通对**远低于截止频率**的振荡几乎不衰减——所以「振荡很慢」时，只拧 \(\tau_{\mathrm{lpf}}\) 往往不够，要回到模型标定（如 \(c\)）等上游病因。

> **工程口诀：** 斜坡链宁慢勿快；平衡环保持快且线性（避免饱和把有效带宽搅乱）。这套 LQG（估计 + LQR）没有纯 LQR 那种一体稳定裕度保证，**频带分离是用工程约束换裕度**。

#### 和「两个控制器」的区别

不是平衡 PID 与坡度 PID 各算各的再仲裁，而是：

- **一个**调节器 \(K\) 管快动态；  
- **一条**慢补偿链改工作点（力 + 姿态参考）。  

口头说「低频控制坡度、高频控制平衡」完全正确；更严谨的说法是：**低频补偿扰动，高频调节状态。**
...
实车开展（S0–S5、HOLD/Trim 门、实车 \(MGR\)/\(K_{\mathrm{EQ}}\)）见 [`stage6.md`](stage6.md)，不在本文重复。
...
| 「平衡和坡度各调各的、越快越好」                          | 应对齐 [频带分离](#freq-separation)：快环平衡、慢链补偿；重叠则自激 |
...
> **落地顺序建议：** 仿真三步（平地退化 → 喂真值 $\alpha$ → 再接 KF）仍然对。实车步骤见 [`stage6.md`](stage6.md)。

---

控制集成卷 · 控制律 $\tau = MGR\cdot\sin_{\mathrm{eff}} - K(\hat{x}-x_{\mathrm{ref}})$，其中 $\sin_{\mathrm{eff}}=k\cdot\mathrm{ramp}\cdot\sin\hat{\alpha}_f$（α̂ 经死区 0.015 rad + 低通 0.5 s 调理）。仿真参数：$M=0.756$、$r=0.0325$、$g=9.8$、$MGR=0.2408$、$K_{\mathrm{EQ}}\approx 1.0$。实车流程与数字见 [`stage6.md`](stage6.md)。定点推导 [#tau-eq-derive](#tau-eq-derive) / [#theta-eq-derive](#theta-eq-derive)；$\hat{\alpha}$ 来源 [slope_kf_simple](./slope_kf_simple.md)。